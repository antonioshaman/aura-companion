...
Aura Companion lets you select a git branch when creating a session and optionally create an isolated git worktree so multiple agents can work on the same repo without interfering with each other.
...
1. Aura Companion creates a new working directory at `~/.companion/worktrees/{repo-name}/{branch}/`
