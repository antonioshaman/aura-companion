...
Check server logs: `aura-companion logs`
...
**Fix**: Aura Companion handles this automatically by creating a unique branch name (e.g., `main-wt-5391`). If you see persistent errors, check `~/.companion/worktrees.json` and remove stale entries.
...
- View server logs for relaunch errors: `aura-companion logs`
...
Aura Companion automatically records all WebSocket messages to JSONL files. Use recordings to debug issues, understand agent behavior, or build test fixtures.
...
```bash
COMPANION_RECORD=0 aura-companion
```
