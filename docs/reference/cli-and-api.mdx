...
| Command | Description |
|---|---|
| `aura-companion` | Start server in foreground (default) |
| `aura-companion serve` | Start server in foreground (explicit) |
| `aura-companion install` | Register as a background service (launchd/systemd) |
| `aura-companion start` | Start the background service |
| `aura-companion stop` | Stop the background service |
| `aura-companion restart` | Restart the background service |
| `aura-companion uninstall` | Remove the background service |
| `aura-companion status` | Show service status |
| `aura-companion logs` | Tail service log files |
...
```bash
# Custom port
aura-companion --port 8080

# Custom auth token
COMPANION_AUTH_TOKEN="my-token" aura-companion

# Install and run as service
aura-companion install && aura-companion start && aura-companion status
...
| `COMPANION_GROUP_RECONNECT_GRACE_MS` | Council pair grace window (ms) before a single-half drop becomes `degraded`. Bounded `[1, 600_000]`; invalid input falls back to default with a structured warn log. | `45000` |
