...
description: Deploy Aura Companion on a private cloud VM with Tailscale access
---

# Deploy on a Cloud VM

Aura Companion runs locally by default, but you can deploy it on a remote server for always-on access, shared team usage, or dedicated development environments.

## Choosing a setup

| Scenario | Recommended approach |
|---|---|
| Solo developer, single machine | [Local install](/get-started/installation) with `bunx aura-companion` |
...
1. Aura Companion server runs on the host (port 3456 by default)
...
This guide walks you through deploying Aura Companion on a private GCP VM in the `europe-west9` (Paris) region. The VM has no public IP — you'll access it securely through Tailscale, which gives you an HTTPS endpoint without exposing anything to the internet.
...
| Component | Purpose |
|---|---|
| GCP Compute Engine VM | Runs Aura Companion (no public IP) |
...
### 6. Install Bun and Aura Companion

SSH into the VM and install Aura Companion as a global package with a background service:
...
# Install Aura Companion globally
bun install -g aura-companion

# Register as a systemd service and start
aura-companion install
aura-companion start
"'
```
...
<Warning>
Before bringing Tailscale up, allowlist the tailnet hostname in Aura Companion. Without `COMPANION_ALLOWED_ORIGIN` set, the server rejects every browser WebSocket upgrade from `*.ts.net` and the UI surfaces `Connection timeout` after `Session started`. See [Origin allowlist](/get-started/installation#remote-access--origin-allowlist-required-for-non-loopback) for the full explanation.

```bash
# On the VM, before step 7
sudo systemctl edit aura-companion
# Drop-in editor opens — paste:
# [Service]
# Environment="COMPANION_ALLOWED_ORIGIN=https://companion.<your-tailnet>.ts.net"
sudo systemctl restart aura-companion
```

Replace `<your-tailnet>` with your actual tailnet name (visible at `tailscale status`).
</Warning>
...
# Serve Aura Companion over HTTPS
...
|---|---|
| `aura-companion status` | Check if the service is running |
| `aura-companion logs` | Tail recent logs |
| `aura-companion restart` | Restart the server |
| `aura-companion stop` | Stop the server |
...
- Aura Companion generates an auth token automatically — see [Authentication](/get-started/installation#authentication) for details
