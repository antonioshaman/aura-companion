...
- `.council-stack-override` contains `aura` AND auto-detection found no opposing-only signal → execute the **Aura** variant of this skill (`/council-plan-aura`). Override takes precedence; auto-detection markers still surface in the enumeration but do not change the verdict.
...
- No override + Aura marker matched + no Python marker matched → execute `/council-plan-aura`.
