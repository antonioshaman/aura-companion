...
This suffixless skill is a **router** — its job is to detect the workspace stack and dispatch to the matching variant. Run this phase BEFORE the council customisation and Phase 1 below. The dispatch path is non-negotiable; if detection fails, refuse loudly. Do NOT silently fall back to either stack.
...
- `.council-stack-override` contains `aura` AND auto-detection found no opposing-only signal → execute the **Aura** variant of this skill (`/council-review-aura`). Override takes precedence; auto-detection markers still surface in the enumeration but do not change the verdict.
- `.council-stack-override` contains `python` AND auto-detection found no opposing-only signal → continue with the Python council body below.
...
- No override + Aura marker matched + no Python marker matched → execute `/council-review-aura`.
...
To override, run:
  /council-review-aura      # if this workspace is the Aura companion
  /council-review            # if this workspace is the Python bot (suffixless variant)
```

After Phase 0 selects a path: if dispatching to the Aura variant, invoke it and stop. If staying in the Python body, proceed to the council customisation and Phase 1 below. If refusing, emit the refusal block exactly as shown and stop.
