...
beforeEach(() => {
  ...
  mockGetImageStatus.mockResolvedValue({ image: "aura-companion-sandbox:latest", status: "ready", progress: [] });
  ...
  mockPullImage.mockResolvedValue({ ok: true, state: { image: "aura-companion-sandbox:latest", status: "pulling", progress: [] } });
...
describe("SandboxManager base image banner", () => {
  ...
  it("shows Not downloaded when base image status is idle", async () => {
    ...
    mockGetImageStatus.mockResolvedValue({ image: "aura-companion-sandbox:latest", status: "idle", progress: [] });
    render(<SandboxManager embedded />);
    await screen.findByText("Base Image");
