...
describe("HomePage", () => {
  ...
  it("renders the title, logo, textarea, and send button", async () => {
    ...
    // Title
    expect(screen.getByText("Aura Companion")).toBeInTheDocument();
  ...
  describe("Sandbox toggle", () => {
    ...
    it("toggles sandbox enabled state on click", async () => {
      ...
      const defaultOption = await screen.findByText("Default (aura-companion-sandbox:latest)");
    ...
    it("shows sandbox dropdown when clicked", async () => {
      ...
      await screen.findByText("Default (aura-companion-sandbox:latest)");
