...
vi.mock("../api.js", () => ({
  api: {
    ...
    pullImage: vi.fn().mockResolvedValue({ ok: true, state: { image: "aura-companion-sandbox:latest", status: "pulling", progress: [] } }),
    getImageStatus: vi.fn().mockResolvedValue({ image: "aura-companion-sandbox:latest", status: "pulling", progress: ["Pulling layer 1..."] }),
  },
}));
...
describe("DockerUpdateDialog", () => {
  ...
  it("triggers image pull when Update is clicked", async () => {
    ...
    expect(mockedApi.pullImage).toHaveBeenCalledWith("aura-companion-sandbox:latest");
  ...
  it("shows done phase when pull completes successfully", async () => {
    ...
    mockedApi.getImageStatus.mockResolvedValue({
      image: "aura-companion-sandbox:latest",
      status: "ready",
      progress: ["Done"],
    });
    useStore.getState().setDockerUpdateDialogOpen(true);
