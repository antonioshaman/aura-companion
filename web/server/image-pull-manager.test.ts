...
const mockGetRegistryImage = vi.hoisted(() => vi.fn((tag: string) => {
  if (tag === "aura-companion-sandbox:latest") return "registry.example.com/aura-companion-sandbox:latest";
...
describe("ImagePullManager", () => {
  ...
  describe("getState", () => {
    it("returns 'ready' when image exists locally", async () => {
      ...
      const state = manager.getState("aura-companion-sandbox:latest");
      expect(state.status).toBe("ready");
      expect(state.image).toBe("aura-companion-sandbox:latest");
    });
  ...
  describe("isReady", () => {
    it("returns true when image exists locally", async () => {
      mockImageExists.mockReturnValue(true);
      const manager = await createManager();
      expect(manager.isReady("aura-companion-sandbox:latest")).toBe(true);
    });

    it("returns false when image does not exist locally", async () => {
      mockImageExists.mockReturnValue(false);
      const manager = await createManager();
      expect(manager.isReady("aura-companion-sandbox:latest")).toBe(false);
    });
  });
  ...
  describe("ensureImage", () => {
    it("starts a background pull when image is missing", async () => {
      ...
      manager.ensureImage("aura-companion-sandbox:latest");
      ...
      await vi.waitFor(() => {
        expect(manager.getState("aura-companion-sandbox:latest").status).toBe("ready");
      });
    ...
    it("is a no-op when image is already being pulled", async () => {
      ...
      manager.ensureImage("aura-companion-sandbox:latest");
      manager.ensureImage("aura-companion-sandbox:latest"); // second call
  ...
  describe("waitForReady", () => {
    it("resolves immediately when image already exists", async () => {
      ...
      const result = await manager.waitForReady("aura-companion-sandbox:latest");
    ...
    it("waits for an in-progress pull to complete", async () => {
      ...
      manager.ensureImage("aura-companion-sandbox:latest");
      const result = await manager.waitForReady("aura-companion-sandbox:latest", 5000);
      expect(result).toBe(true);
    });
    ...
    it("times out when pull takes too long", async () => {
      ...
      const result = await manager.waitForReady("aura-companion-sandbox:latest", 50);
      expect(result).toBe(false);
    });
  });
  ...
  describe("pull (force re-pull)", () => {
    it("triggers a pull even when image is already present", async () => {
      ...
      manager.pull("aura-companion-sandbox:latest");
  ...
  describe("onProgress", () => {
    it("fires callback for each progress line during pull", async () => {
      ...
      const manager = await createManager();
      const unsub = manager.onProgress("aura-companion-sandbox:latest", (line) => lines.push(line));
  ...
  describe("fallback to local build", () => {
    it("falls back to local build when pull fails for aura-companion-sandbox:latest", async () => {
      mockImageExists.mockReturnValue(false);
      mockPullImage.mockResolvedValue(false);
      mockExistsSync.mockReturnValue(true);
  ...
  describe("initFromEnvironments", () => {
    it("is a no-op since environments no longer carry Docker fields", async () => {
      ...
      mockListEnvs.mockReturnValue([
        { name: "env1", slug: "env1", baseImage: "aura-companion-sandbox:latest", variables: {}, createdAt: 0, updatedAt: 0 },
