...
// ─── Council Mode — sendUserFrameFromServer (observer wake) ───────────────
//
// CodexAdapter mirrors ClaudeAdapter.sendUserFrameFromServer for the
// council wake-dispatcher. Closes Residual #1 from HANDOFF-morning.md:
// prior to this, `ws-bridge.sendObserverWakeFrame` returned
// `unsupported_backend` for any Codex observer, silently skipping the
// wake and forcing every claude+codex pair onto the user-relay fallback.
// Test surface mirrors the corresponding Claude test paths in
// claude-adapter.test.ts; each gate (initialised, transport, turn-state)
// has its own assertion so a future refactor that loses a gate goes red.

describe("CodexAdapter — sendUserFrameFromServer (Council Mode wake)", () => {
  // Inline helper — same shape as createMockTransport above but local
  // to keep this block self-contained for future split into a dedicated
  // wake-test file.
  function createMockTransport() {
    let nextCallId = 0;
    const pendingCalls = new Map<number, { resolve: (v: unknown) => void; reject: (e: Error) => void }>();
    ...
    return {
      ...
      resolveCall(n: number, result: unknown) {
        const pending = pendingCalls.get(n);
        if (pending) { pendingCalls.delete(n); pending.resolve(result); }
      },
      rejectCall(n: number, err: Error) {
        const pending = pendingCalls.get(n);
        if (pending) { pendingCalls.delete(n); pending.reject(err); }
      },
    };
  }

  /** Drive the adapter through init → thread/start → rateLimits so it
   *  reaches the post-init state where wake gates are exercised. */
  async function initAdapter(adapter: CodexAdapter, mock: ReturnType<typeof createMockTransport>) {
    await new Promise((r) => setTimeout(r, 30));
    // Resolve in order: initialize, thread/start, rateLimits.
    mock.resolveCall(1, { userAgent: "codex" });
    await new Promise((r) => setTimeout(r, 20));
    mock.resolveCall(2, { thread: { id: "thr_wake_test" } });
    await new Promise((r) => setTimeout(r, 30));
    mock.resolveCall(3, {});
    await new Promise((r) => setTimeout(r, 20));
  }

  it("returns {kind: 'sent'} on the happy path and dispatches turn/start", async () => {
    const mock = createMockTransport();
    const adapter = new CodexAdapter(mock.transport, "obs-codex-1", { model: "o4-mini", cwd: "/tmp/work" });
    await initAdapter(adapter, mock);

    const outcome = adapter.sendUserFrameFromServer("review checkpoint 7");
    expect(outcome).toEqual({ kind: "sent" });

    // The wake should have dispatched a turn/start with the right shape.
    // Calls so far: initialize, thread/start, account/rateLimits/read,
    // and our new turn/start — index 3 (0-indexed) is the wake call.
    const wakeCall = mock.calls.find((c) => c.method === "turn/start");
    expect(wakeCall).toBeDefined();
    expect(wakeCall!.params).toMatchObject({
      threadId: "thr_wake_test",
      input: [{ type: "text", text: "review checkpoint 7" }],
    });
    expect(wakeCall!.params.approvalPolicy).toBeDefined();
    expect(wakeCall!.params.sandboxPolicy).toBeDefined();
  });

  it("returns {kind: 'socket_disconnected'} when adapter not yet initialised", () => {
    const mock = createMockTransport();
    const adapter = new CodexAdapter(mock.transport, "obs-codex-pre-init", { model: "o4-mini" });
    // Do NOT call initAdapter — adapter is mid-init, no threadId.

    const outcome = adapter.sendUserFrameFromServer("early wake");
    expect(outcome).toEqual({ kind: "socket_disconnected" });
  });

  it("returns {kind: 'socket_disconnected'} when transport is closed", async () => {
    const mock = createMockTransport();
    const adapter = new CodexAdapter(mock.transport, "obs-codex-tx-down", { model: "o4-mini" });
    await initAdapter(adapter, mock);

    // Simulate transport drop after init.
    (mock.transport.isConnected as ReturnType<typeof vi.fn>).mockReturnValue(false);
    const outcome = adapter.sendUserFrameFromServer("wake while disconnected");
    expect(outcome).toEqual({ kind: "socket_disconnected" });
  });

  it("returns {kind: 'busy'} when an in-flight turn is already running", async () => {
    const mock = createMockTransport();
    const adapter = new CodexAdapter(mock.transport, "obs-codex-busy", { model: "o4-mini" });
    await initAdapter(adapter, mock);

    // First wake — should go through and (when resolved) set currentTurnId.
    const first = adapter.sendUserFrameFromServer("first wake");
    expect(first).toEqual({ kind: "sent" });

    // Resolve turn/start to populate currentTurnId, then attempt second wake.
    const wakeCallIdx = mock.calls.findIndex((c) => c.method === "turn/start");
    expect(wakeCallIdx).toBeGreaterThan(-1);
    // turn/start is the (wakeCallIdx+1)-th call by 1-based index.
    mock.resolveCall(wakeCallIdx + 1, { turn: { id: "turn_in_flight" } });
    await new Promise((r) => setTimeout(r, 10));

    const second = adapter.sendUserFrameFromServer("second wake while busy");
    expect(second).toEqual({ kind: "busy" });
  });

  it("does NOT propagate async turn/start rejection back to the caller", async () => {
    // Wake is fire-and-forget — the synchronous return reports gate
    // outcome, and an async rejection of the turn/start RPC must not
    // escape to crash the dispatcher's microtask queue.
    const mock = createMockTransport();
    const adapter = new CodexAdapter(mock.transport, "obs-codex-async-fail", { model: "o4-mini" });
    await initAdapter(adapter, mock);

    const outcome = adapter.sendUserFrameFromServer("wake that fails async");
    expect(outcome).toEqual({ kind: "sent" });

    // Reject the wake's turn/start call.
    const wakeCallIdx = mock.calls.findIndex((c) => c.method === "turn/start");
    expect(wakeCallIdx).toBeGreaterThan(-1);
    let unhandled = false;
    ...
    try {
      mock.rejectCall(wakeCallIdx + 1, new Error("simulated codex server overload"));
      // Give the microtask + nextTick a chance to surface any unhandled error.
      await new Promise((r) => setTimeout(r, 30));
