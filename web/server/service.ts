...
const UNIT_NAME = "aura-companion.service";
...
export function generateSystemdUnit(opts: UnitOptions): string {
  ...
  return `[Unit]
Description=Aura Companion - Web UI for Claude Code
...
function resolveBinPath(): string {
  try {
    const binPath = execSync("which aura-companion", { encoding: "utf-8" }).trim();
    if (binPath) return binPath;
  } catch {
    // not found globally
  }

  console.error("aura-companion must be installed globally for service mode.");
  console.error("");
  console.error("  bun install -g aura-companion");
  console.error("");
  console.error("Then retry:");
  console.error("");
  console.error("  aura-companion install");
  process.exit(1);
}
...
async function installDarwin(opts?: { port?: number }): Promise<void> {
  ...
  if (existsSync(PLIST_PATH)) {
    console.error("Aura Companion is already installed as a service.");
    console.error("Run 'aura-companion uninstall' first to reinstall.");
    process.exit(1);
  }
  ...
  console.log("Aura Companion has been installed as a background service.");
  console.log("");
  ...
  console.log("Use 'aura-companion status' to check if it's running.");
}
...
async function uninstallDarwin(): Promise<void> {
  ...
  console.log("Aura Companion service has been removed.");
...
async function startDarwin(): Promise<void> {
  ...
  if (!installedService) {
    console.log("Aura Companion is not installed as a service.");
    console.log("Run 'aura-companion install' first.");
    return;
  }
  ...
  console.log("Aura Companion service has been started.");
}
...
async function stopDarwin(): Promise<void> {
  ...
  console.log("Aura Companion service has been stopped.");
  console.log("Run 'aura-companion restart' to start it again.");
}
...
async function restartDarwin(): Promise<void> {
  ...
  console.log("Aura Companion service has been restarted.");
}
