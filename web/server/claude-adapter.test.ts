...
describe("Known non-standard CLI message types", () => {
  ...
  it("user echo with tool_result array content is silently dropped", () => {
    // After every assistant tool_use, the SDK sends a user-role message
    // containing tool_result blocks for the model context. These echoes
    // duplicate output already shown in the UI via the assistant tool_use
    // ToolBlock + tool_progress events, so forwarding them as user_message
    // would render raw JSON dumps in the chat. Drop them silently.
    const ws = createMockSocket("sess-1");
    adapter.attachWebSocket(ws);
...
/**
 * PR #177 — Silent-stdio recovery: `handleSystemInit` replays the last
 * unanswered user message when a fresh `system.init` frame arrives.
 * Guarded by the pending-content field which is:
 *   - set at the end of `handleOutgoingUserMessage` (text-only turns),
 *   - cleared on the in-flight → awaiting-input transition in
 *     `handleResultMessage`,
 *   - preserved across `handleTransportClose` so the next spawn's
 *     init frame can drain it.
 */
describe("Silent-stdio recovery — last-user-message replay on respawn", () => {
  function newFixture() {
    const localBrowserCb = vi.fn();
    const a = new ClaudeAdapter("sess-replay");
    a.onBrowserMessage(localBrowserCb as any);
    const socket = createMockSocket("sess-replay");
    a.attachWebSocket(socket);
    return { a, socket, browserCb: localBrowserCb };
  }

  /** Extract the JSON payload from every `.send()` call on the mock. */
  function allSent(socket: ReturnType<typeof createMockSocket>): any[] {
    return socket.send.mock.calls.map((c: any[]) => JSON.parse((c[0] as string).trim()));
  }

  it("replays a text-only user_message on the NEXT init frame if the previous turn was cut off", () => {
    // Real 2026-09-09 shape: user sends → CLI receives on stdin →
    // stdout goes silent → watchdog kills → keepalive respawns via
    // --resume → fresh init lands. The message content the user sent
    // (and was owed a reply for) MUST re-drive on the new spawn or the
    // user's turn is silently lost.
    const { a, socket, browserCb } = newFixture();
    // Deliver the initial init so the first user_message dispatches
    // rather than getting queued behind pendingMessages.
    a.handleRawMessage(makeInitMsg({ session_id: "cli-init-1" }));
    socket.send.mockClear();

    a.send({ type: "user_message", content: "please continue the plan" });
    // Sanity: the original send did fire on the transport.
    expect(allSent(socket).some((m) => m.type === "user" && m.message.content === "please continue the plan")).toBe(true);
    socket.send.mockClear();

    // No `result` frame arrives — the silence-watchdog kill+respawn
    // simulation is: transport closes, then a fresh init lands on the
    // next attach. The adapter's pending-content field survives.
    a.handleTransportClose();
    a.attachWebSocket(createMockSocket("sess-replay"));
    // Re-wire the transport reference so getLastSent still points at
    // the real socket (attachWebSocket in production would supply the
    // new socket; the fixture keeps the original `socket` for the
    // assertion below).
    (a as any).transport = { send: socket.send, isOpen: () => true, bufferedAmount: () => 0, raw: null, markClosed: () => {} };

    // Fresh init on the new spawn → replay fires.
    a.handleRawMessage(makeInitMsg({ session_id: "cli-init-2" }));

    const sent = allSent(socket);
    const replayed = sent.filter((m) => m.type === "user" && m.message.content === "please continue the plan");
    expect(replayed.length).toBe(1);

    // Also surfaces the browser-visible recovery notice so the user
    // knows their message is being re-driven, not just gone quiet.
    const notices = browserCb.mock.calls.map((c: any[]) => c[0]);
    const recovery = notices.find((n: any) => n.type === "error" && /replaying your last message/i.test(n.message));
    expect(recovery).toBeDefined();
  });

  it("does NOT replay after a clean `result` frame — turn was answered", () => {
    const { a, socket } = newFixture();
    a.handleRawMessage(makeInitMsg({ session_id: "cli-clean-1" }));
    socket.send.mockClear();

    a.send({ type: "user_message", content: "one-shot question" });
    // Real answer arrives + terminator result.
    a.handleRawMessage(makeAssistantMsg());
    a.handleRawMessage(makeResultMsg());
    socket.send.mockClear();

    // Simulate respawn: transport close + fresh init on new attach.
    a.handleTransportClose();
    a.attachWebSocket(createMockSocket("sess-replay"));
    (a as any).transport = { send: socket.send, isOpen: () => true, bufferedAmount: () => 0, raw: null, markClosed: () => {} };
    a.handleRawMessage(makeInitMsg({ session_id: "cli-clean-2" }));

    // No replay — the pending field was cleared by the successful result.
    const sent = allSent(socket);
    const spurious = sent.filter((m) => m.type === "user" && m.message.content === "one-shot question");
    expect(spurious.length).toBe(0);
  });

  it("skips replay for messages carrying images (out of scope)", () => {
    const { a, socket } = newFixture();
    a.handleRawMessage(makeInitMsg({ session_id: "cli-img-1" }));
    socket.send.mockClear();
    ...
    a.attachWebSocket(createMockSocket("sess-replay"));
    (a as any).transport = { send: socket.send, isOpen: () => true, bufferedAmount: () => 0, raw: null, markClosed: () => {} };
    a.handleRawMessage(makeInitMsg({ session_id: "cli-img-2" }));

    // Image-carrying turns are NOT stored for replay by design —
    // re-decoding + re-transmitting a base64 blob on our own initiative
    // is a larger blast radius than the anti-silence win.
    const sent = allSent(socket);
    const anyUser = sent.filter((m) => m.type === "user");
    expect(anyUser.length).toBe(0);
  });

  it("replay loop is bounded — every fresh init re-arms silence deadline via handleOutgoingUserMessage", () => {
    // The design intent: kill → respawn → replay → same silence again
    // → kill → respawn → replay → … will trip MAX_AUTO_RELAUNCHES on
    // the orchestrator side, NOT loop indefinitely at the adapter
    // layer. This test asserts the adapter's contribution: after each
    // replay, `lastUnansweredUserMessage` is re-set (so a subsequent
    // silence-driven respawn CAN in principle replay again — but the
    // orchestrator's retry budget is the cap).
    const { a, socket } = newFixture();
    a.handleRawMessage(makeInitMsg({ session_id: "cli-loop-0" }));
    a.send({ type: "user_message", content: "long-running turn" });

    // First respawn.
    a.handleTransportClose();
    a.attachWebSocket(createMockSocket("sess-replay"));
    (a as any).transport = { send: socket.send, isOpen: () => true, bufferedAmount: () => 0, raw: null, markClosed: () => {} };
    a.handleRawMessage(makeInitMsg({ session_id: "cli-loop-1" }));

    // Second respawn: without any `result` in between, the second init
    // should ALSO replay (the field was re-set inside the first replay).
    a.handleTransportClose();
    a.attachWebSocket(createMockSocket("sess-replay"));
    (a as any).transport = { send: socket.send, isOpen: () => true, bufferedAmount: () => 0, raw: null, markClosed: () => {} };
    a.handleRawMessage(makeInitMsg({ session_id: "cli-loop-2" }));

    const replays = allSent(socket).filter((m) => m.type === "user" && m.message.content === "long-running turn");
    // Expect ≥ 2 dispatches: one original + at least one replay across the two respawns.
